...
annuityAllocation <- sample(seq(0,1,.1),n,replace=TRUE)   # randomize annuity allocations from zero to 100%
...
diaAllocation <- annuityAllocation * (1 - annuitySampl)

# Set immediate annuity parameters

purchaseAgeSPIA <- 65          # age of SPIA owner when purchased
annuityStartAge <- 66          # age of annuitant when payments will begin
annPmt <- 800 * 12             # quote for annual payment before any deaths
inflationProtSPIA <- TRUE      # annuity is inflation protected
spiaOwnerisHusband <- TRUE     # = 1 if husband is annuitant and wife is sur
spiaPayout <- .0536            # payout of fixed annuity, male age 65 joint female age 65
survivorPercentSPIA <- .5      # percent SPIA annuity survivor benefit

# set deferred income annuity parameters

purchaseAgeDIA <- 65         #age at which DIA will be purchased
diaPayout <- 21420           # annual payout for this diaPayout (see quotes at abaris.com)
startAgeDIA <- 80            # annuity owners age when payouts will begin
diaOwnerisHusband <- TRUE    # TRUE or FALSE
survivorPercentDIA <- .5     # % of payout paid to a surviving spouse
inflationProtDIA <- TRUE    # payouts are inflation-protected, TRUE or FALSE
...
annuityIncomeM <- buildSpiaM (spiaAllocation,spiaPayout,states,purchaseAgeSPIA,spiaOwnerisHusband,survivorPercentSPIA,inflationProtSPIA,market.cumCsM,husbandAge,wifeAge,saveInitPort) 
...
spendFromPortfolio <- max(0,spendM - socSecM - annuityIncomeM)
