...
* Immediate Annuity (SPIA) parameters
    ...
    + Annuity is inflation-protected is `r inflationProtSPIA`.
    + Husband owns Annuity is `r as.logical(spiaOwnerisHusband)`.
    + Percent of benefit that goes to survivor is `r round(100*survivorPercentSPIA,2)`%.
    + SPIA payout rate is `r round(100*spiaPayout,2)`%.
...
* Spending parameters
    + Expected spending year one of retirement randomized from `r paste("$",prettyNum(min(unique(initspend)),scientific=FALSE,big.mark=","))` to `r paste("$",prettyNum(max(unique(initspend)),scientific=FALSE,big.mark=","))`.
...
if (length(unique(equityAllocation) & unmetSpendIndex > 0) > 1) {
...
if (sum(percentFunded) < n) {     # plot only if some scenarios are not 100% funded
