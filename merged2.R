...
userDataFile <- "RISMAT Input Butts.csv"
...
source('~/R Projects/RetirementModel/InputFromXL.R')
...
equityAllocation <- sample(seq(.3,.6,.1),n,replace=TRUE)   # .3,.5,.05 randomize equity allocations from zer0 to 100%
...
annuityAllocation <- sample(seq(0,0,0),n,replace=TRUE)   # randomize annuity allocations from zero to 100%
annuitySampl <- sample(seq(0,1,.1),n,replace=TRUE)        # randomly allocate annuities among DIAs and SPIAs
spiaAllocation <- annuityAllocation # * annuitySampl
diaAllocation <- 0 # annuityAllocation  * (1 - annuitySampl)
...
# survivorPercentSPIA is percent SPIA annuity survivor benefit
...
if (husbandAge > purchaseAgeSPIA){ # yes   #### fix for wife is owner
  purchaseAgeSPIA <- husbandAge
} else {
...
###########################################
# Build TIPS Ladder Income Matrix for Wife
###########################################
...
# annuityIncomeM <- annuityIncomeM + tipsIncomeM
...
# spendFromPortfolio[spendFromPortfolio < 0] <- 0 # change any negative portfolio spending to zero
  ...
  if (printSummary == TRUE) {
    ...
    for (scenarioNumber in 1:scenarios) {
      ...
      if (states[scenarioNumber,yearOfRetirement-1] == 4) {   # surviving spouse died last year
        ...
        termNetWorth[scenarioNumber] <- termHomeEquity[scenarioNumber] + termPortValue[scenarioNumber] + ladderTV[yearOfRetirement]
        ...
        # if safe income sources exceed expenses, reduce safe income spending (it has already been transferred to the investment portfolio)
        # if (annuityIncomeM[scenarioNumber,yearOfRetirement] + socSecM[scenarioNumber,yearOfRetirement] > expenses[scenarioNumber,yearOfRetirement]) socSecM[scenarioNumber,yearOfRetirement] <- socSecM[scenarioNumber,yearOfRetirement] - (annuityIncomeM[scenarioNumber,yearOfRetirement] + socSecM[scenarioNumber,yearOfRetirement] - expenses[scenarioNumber,yearOfRetirement])
        # if ( socSecM[scenarioNumber,yearOfRetirement] < 0) {        # need to reduce safe income spending, too. Soc Sec not enough
        #   annuityIncomeM[scenarioNumber,yearOfRetirement] <- annuityIncomeM[scenarioNumber,yearOfRetirement] + socSecM[scenarioNumber,yearOfRetirement]
        #   socSecM[scenarioNumber,yearOfRetirement] <- 0   # unspent all of Social Security and needed to unspend more from safe income
        # }
